...
import { CalendarPlus2, Download, Search } from "lucide-react";
    ...
    <header className="flex flex-col w-full z-30">
      <div className="flex items-end justify-between w-full border-b border-zinc-300 dark:border-zinc-700 pb-2">
        <div className="flex items-center gap-1.5 text-zinc-600 dark:text-zinc-400">
          {tabs.map((tab, i) => (
            <Link
              to={`/schedules/${tab === "All" ? "" : tab.toLowerCase()}`}
              key={i}
        ...
        <div className="flex items-center gap-1 lg:gap-4">
          ...
          >
            <CalendarPlus2 className="w-5" />
            <p className="hidden lg:block">Add Schedules</p>
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 items-center pt-2 text-sm">
        {tableHeaders.map((filter, i) => {
          ...
          if (filter.name === "Doctor Name") options = doctorOptions;
